...
export class KineticScroller {
  ...
  onScroll(pointer: Phaser.Pointer, x: number, y: number) {
    const cameraPosition = Phaser.Point.add(
      this.startCameraPosition,
      Phaser.Point.subtract(this.startScrollPosition, pointer.position)
    );
    ...
    this.scrollVelocity = Phaser.Point.subtract(pointer.position, this.previousScrollPosition).divide(
      elapsedSeconds,
      elapsedSeconds
    );
  ...
  onScrollEnd() {
    if (this.scrollVelocity) {
      ...
      const cameraCenter = this.phaserStore.phaser.camera.position
        .clone()
        .add(this.phaserStore.phaser.camera.width / 2, this.phaserStore.phaser.camera.height / 2);
